...
//------------------------------------------------------------------------------
bool OpenCL_ConstantDouble(cl_float Argument, float Value){
 cl_float error = clSetKernelArg(Kernel, Argument, sizeof(float), &Value);
 if(error != CL_SUCCESS){
  printf("Error: Failed set float constant.\n");
  return false;
 }
 return true;
}
